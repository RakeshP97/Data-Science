#Task 4: Eta values takes from range 0.01 to 0.05 by looping 100 epochs and printing Each eta value traing MSE and TEST MSE
#Load Auto Data
...
for (e in 1:5)
{
  ...
  for (j in 1:100)
  {
    ...
    # Update the weights
    beta_values[1] = beta_values[1] + (n[e] * (sum(d) / dim(new_train.X)[1]))
    for (k in 1:5)
    {
      dx <- 0
      ...
      beta_values[k + 1] = beta_values[k + 1] + ((n[e] / (dim(new_train.X)[1])) * (dx))
    }
  ...
  TEST_MSE <- TEST_MSE / train
  test_mse[e]<- TEST_MSE
  print(c(n[e], MSE, TEST_MSE))
}
plot(n,test_mse,col='red', type='l')
